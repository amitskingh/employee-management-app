...
import { TitleCasePipe, NgClass, NgStyle, UpperCasePipe } from '@angular/common';
...
export class EmployeeList implements AfterViewInit {
  ...
  viewRow(row: any) {
    this.router.navigate([`/employees/view/${row.id}`], row.id);
    console.log(row.id);
  }
  ...
  editRow(row: any) {
    console.log(row);
    this.router.navigate([`/employees/edit/${row.id}`], row.id);
  }
